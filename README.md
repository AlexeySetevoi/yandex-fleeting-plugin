...
`Increase` ждёт завершения операции создания (около 35–45 секунд на ВМ), иначе нехватку ресурсов зоны не увидеть.
Побочный эффект: если раннер просит машины по одной, а не пачкой, вторая начинает создаваться только после того, как
создана первая. Несколько ВМ из одного запроса создаются параллельно.
