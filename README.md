...
Если упёрлись в квоту каталога, фолбэк не поможет — квота общая на все зоны.
...
Metadata `ssh-keys` не работает, если для ВМ включён OS Login (на уровне организации) — для раннер-каталога его
нужно выключить.
...
Фолбэк по размещениям — первым ставится вариант, в котором ресурсов заведомо нет (зона или платформа, недоступная
вашему облаку):
...
Если облако отвечает на такой вариант не `RESOURCE_EXHAUSTED`, а другим кодом, перехода к следующему не будет — код
виден в ошибке `Increase`, условие перехода задаётся в `mapError` (`internal/yandexapi/sdk.go`).
